package model
{
	...
	public class SkeletonXMLProxy
	{
		...
		public function changePath():void
		{
			...
			for each(var animationXML:XML in getAnimationXMLList(_skeletonXML))
			{
				var subTextureName:String = animationXML.attribute(ConstValues.A_NAME);
				subTextureName = subTextureName.split("/").join("-");
				animationXML.@[ConstValues.A_NAME] = subTextureName;
			}
			
			
			for each(var armatureXML:XML in getArmatureXMLList(_skeletonXML))
			{
				var subTextureName:String = armatureXML.attribute(ConstValues.A_NAME);
				subTextureName = subTextureName.split("/").join("-");
				armatureXML.@[ConstValues.A_NAME] = subTextureName;
			}
