	...
	public class BaseFactory extends EventDispatcher
	{
		...
		protected function buildBone(boneData:BoneData):Bone
		{
			...
			for(var i:int = boneData._displayNames.length - 1;i >= 0;i --)
			{
				...
				//JSFLProxy.jsflTrace("buildBone:"+displayName);
				...
				{
					if(displayName=="LabelType"){ 
						bone.display = generateLabelDisplay(displayData, displayName, displayData.pivotX, displayData.pivotY);
					}else{
						bone.display = generateTextureDisplay(_currentTextureAtlas, displayName, displayData.pivotX, displayData.pivotY);
						
					}
				}
				//JSFLProxy.jsflTrace("buildBone:"+displayName+"  end");
			}
			return bone;
		}
		/** @private */
		...
		protected function generateLabelDisplay(data:DisplayData, fullName:String, pivotX:Number, pivotY:Number):Object
		{
			...
			format.color=parseInt(data.color.substring(1, data.color.length), 16);
