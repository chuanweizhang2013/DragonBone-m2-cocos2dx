...
//							writeByte.writeUTFBytes(mergeSkeletonAndTextureXML().toXMLString());
//							exportSave(
//								writeByte,
//								_importDataProxy.skeletonName+GlobalConstValues.XML_SUFFIX 
//								
//							);
							ExportExpandDataCommand.xmlData=mergeSkeletonAndTextureXML();
							MessageDispatcher.addEventListener(MessageDispatcher.ALL_ITEM_ATTR_GOT, readAllAttrHandler);
							LoadFLADataCommand.instance.readAllAttr();
							return;
						}
						catch(_e:Error)
						{
		...
		public function readAllAttrHandler(e:Message):void{
			ExportExpandDataCommand.expandData=  e.parameters[0] as XML;
			exportData(ExportExpandDataCommand.getMergeData());
		}
		public function exportData(data:String):void{
			var writeByte:ByteArray=new ByteArray();
			writeByte.writeUTFBytes(ExportExpandDataCommand.getMergeData());
			exportSave(
				writeByte,
				_importDataProxy.skeletonName+GlobalConstValues.XML_SUFFIX 
		...
		public function exportDirectData(data:String):void{
			var writeByte:ByteArray=new ByteArray();
			writeByte.writeUTFBytes(data);
			exportSave(
				writeByte,
				_importDataProxy.skeletonName+GlobalConstValues.XML_SUFFIX 
