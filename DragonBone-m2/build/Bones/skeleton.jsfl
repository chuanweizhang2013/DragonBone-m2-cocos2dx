...
var A_TEXT = "text";
var A_FONT_SIZE = "size";
var A_FONT_NAME= "font";
var A_FONT_COLOR= "color";
var A_FONT_ALIGN= "align";
...
function isArmatureItem(_item, _isChildArmature){
	if(_item.symbolType != MOVIE_CLIP && _item.symbolType != GRAPHIC && !isLabel(_item) )
	{
		return null;
	}
	var _layersFiltered = [];
...
function getMovementXML(_movementName, _duration, _item){
	...
	if(_movementXML){
		_xml.@[A_DURATION_TO] = 0;// _movementXML.@[A_DURATION_TO];
	}else{
		_xml.@[A_DURATION_TO] = 0;
	}
	if(_duration > 1){
		if(_movementXML){
			...
			_xml.@[A_LOOP] =1;// _movementXML.@[A_LOOP];
...
function generateMovement(_item, _mainFrame, _layers){
	//fl.trace("generateMovement:"+_item.name);
	...
	for each(var _layer in _layers){
     ...
     //fl.trace("generateMovement _movementBoneXML"+_movementBoneXML.toXMLString());
...
function generateFrame(_frame, _boneName, _symbol, _z, _noAutoEasing){
	...
	if(_symbol.blendMode!="normal"){
		
		if(_symbol.blendMode=="layer"){
			_frameXML.@[A_BLEND] =1;
		}else if(_symbol.blendMode=="multiply"){ 
			_frameXML.@[A_BLEND] =3; 
		}else if(_symbol.blendMode=="screen"){ 
			_frameXML.@[A_BLEND] =5; 
		}else if(_symbol.blendMode=="overlay"){ 
			_frameXML.@[A_BLEND] =6; 
		}else if(_symbol.blendMode=="hardlight"){ 
			_frameXML.@[A_BLEND] =7; 
		}else if(_symbol.blendMode=="lighten"){
			_frameXML.@[A_BLEND] =4; 
		}else if(_symbol.blendMode=="darken"){ 
			_frameXML.@[A_BLEND] =2; 
		}else if(_symbol.blendMode=="difference"){
			_frameXML.@[A_BLEND] =10; 
		
		}else if(_symbol.blendMode=="add"){
		
			_frameXML.@[A_BLEND] =8; 
		}else if(_symbol.blendMode=="subtract"){
		
			_frameXML.@[A_BLEND] =9; 
		}else if(_symbol.blendMode=="invert"){
		
			_frameXML.@[A_BLEND] =11; 
		}else if(_symbol.blendMode=="alpha"){ 
			_frameXML.@[A_BLEND] =12; 
		
		}else if(_symbol.blendMode=="erase"){ 
			_frameXML.@[A_BLEND] =13; 
	...
	if(isLabel(_symbol)){
		  _imageName="LabelType"
		_displayXML.@[A_WIDTH] = Math.ceil(_symbol.width);
		_displayXML.@[A_HEIGHT] = Math.ceil(_symbol.height);
		_displayXML.@[A_TEXT]=_symbol.getTextString();
		_displayXML.@[A_FONT_SIZE]=_symbol.getTextAttr("size");
		_displayXML.@[A_FONT_COLOR]=_symbol.getTextAttr("fillColor");
		_displayXML.@[A_FONT_ALIGN]=_symbol.getTextAttr("alignment");
		_frameXML.@[A_X] = formatNumber(_symbol.x  );
		_frameXML.@[A_Y] = formatNumber(_symbol.y );
		
		_frameXML.@[A_PIVOT_X] =0;
		_frameXML.@[A_PIVOT_Y] =0;
...
//fl.trace("getArmatureList")
	...
	fl.trace(_xml.toXMLString());
	return _xml.toXMLString();
}

function getStageItem(curName){
	...
	for(var i=0;i<doc.selection.length;i++){  
		if(doc.selection[i].libraryItem==null){
			continue	
		}
  	 if(doc.selection[i].libraryItem.name==curName){
...
dragonBones.generateArmature = function(_armatureName, _scale, _isNewXML, _isChildArmature){
	//fl.trace("generateArmature:"+_armatureName);
	...
	if(getStageItem(_armatureName)){
		curScaleX=getStageItem(_armatureName).scaleX;
		curScaleY=getStageItem(_armatureName).scaleY; 
	} 
	...
	if(_mainLayer.frameCount >= 1 || _mainLayer.name=="action"){
...
dragonBones.addTextureToSWFItem = function(_textureName, _isLast){
	...
	//fl.trace("addTextureToSWFItem:"+_textureName);
...
dragonBones.initStand=function(){ 
var doc=fl.getDocumentDOM();  
var lib=fl.getDocumentDOM().library;  
var tl=fl.getDocumentDOM().getTimeline();  
	...
	doc.exitEditMode();  
	lib.selectAll()
	var _items = lib.getSelectedItems()
	 
	for each(var _item in _items){ 
       if(_item.itemType =="folder"  || _item.itemType =="bitmap"){ 
		   continue;
	   }
	    
	    if(_item.name.indexOf("images/")!=-1)
		{
			
		   continue;
		}
		  if(_item.name.indexOf("textureSWFItem")!=-1)
		{
			
		   continue;
		}
		//fl.trace(_item.name)
	lib.selectItem(_item.name);
	lib.editItem(_item.name); 
 
	var layer=null;
	for(var j=0;j<doc.getTimeline().layers.length;j++){
		if(doc.getTimeline().layers[j].frames[0] && doc.getTimeline().layers[j].frames[0].name=="stand"){
			doc.getTimeline().deleteLayer(j);
			break;
		} 	
	}
	
	for(var j=0;j<doc.getTimeline().layers.length;j++){
		var frames=doc.getTimeline().layers[j].frames
	...
	doc.getTimeline().addNewLayer("script") 
	doc.getTimeline().layers[doc.getTimeline().currentLayer].frames[0].name="stand"; 
	doc.getTimeline().layers[doc.getTimeline().currentLayer].frames[0].labelType="name";  
...
dragonBones.initGuide=function(){ 
//fl.trace("initGuide")
var doc=fl.getDocumentDOM();  
var lib=fl.getDocumentDOM().library;  
var tl=fl.getDocumentDOM().getTimeline();  
...
for(var i=0;i<doc.selection.length;i++){ 
	...
	lib.selectItem(doc.selection[i].libraryItem.name);
	lib.editItem(doc.selection[i].libraryItem.name); 
  
	for(var j=0;j<doc.getTimeline().layers.length;j++){
		if(doc.getTimeline().layers[j].parentLayer &&
		   doc.getTimeline().layers[j].parentLayer.layerType=="guide"){
			doc.getTimeline().setSelectedLayers(j);
			for(var k=1;k<doc.getTimeline().layers[j].frames.length-1;k++){
				fl.getDocumentDOM().getTimeline().insertKeyframe(k) 
			} 
		} 	
	}
 ...
 dragonBones.getUIEditorAttr=function(mcName,item,key){ 
	var doc=fl.getDocumentDOM();  
	var lib=fl.getDocumentDOM().library;  
	var tl=fl.getDocumentDOM().getTimeline();  
	mc=getUIEditorMc(mcName);
	if(mc.actionScript==""){
		 xmlData= <attr/>
	}else{ 
		 var itemName=item;
		 xmlData=new XML(mc.actionScript); 
	} 
	
	itemXml=getUIEditorAttrItems(xmlData,item,key); 
	fl.trace("js getUIEditorAttr:"+mcName+","+item+","+key+","+itemXml.toXMLString());
	return itemXml.toXMLString();
	 
}



function getUIEditorAttrItems(xmlData,item,key){
	var doc=fl.getDocumentDOM();  
	var lib=fl.getDocumentDOM().library;  
	var tl=fl.getDocumentDOM().getTimeline();  
	
	var ret= <ret/>
	for each(var nodeItem in xmlData["item"]){
		 if(nodeItem.@name==item ){ 
		 	ret.appendChild(nodeItem);
		 } 
	}
...
dragonBones.exportDirect=function(){ 
	currentDom = fl.getDocumentDOM();
	dragonBones.initStand()
	dragonBones.initGuide()
	var _resultXML = XML(dragonBones.getArmatureList());
	var _flaDomName = _resultXML.attribute( A_NAME);
	_armatureXMLList = _resultXML.elements( ARMATURE);


	var _resultXML = XML(dragonBones.getArmatureList());
	ret = <{SKELETON} {A_NAME}={_flaDomName} {A_FRAME_RATE}={currentDom.frameRate} version="2.2" />;
	armaturesXML = <{ARMATURES}/>;
	animationsXML = <{ANIMATIONS}/>;
	textureAtlasXML = <{TEXTURE_ATLAS}  />;
	ret.appendChild(armaturesXML);
	ret.appendChild(animationsXML);
	ret.appendChild(textureAtlasXML);

	for(var i=0;i<_armatureXMLList.length();i++){
		var  xml=XML( dragonBones.generateArmature(_armatureXMLList[i].@name,_armatureXMLList[i].@scale,true))
		armaturesXML.appendChild(xml.armatures[0].armature[0]) 
		animationsXML.appendChild(xml.animations[0].animation[0])
		
    	armature=xml.armatures[0].armature[0]
		for(var j=0;j<armature.b.length();j++){
			if(j<armature.b[j].@isArmature=="1"){
			
			}else{ 
				for(var k=0;k<armature.b[j].d.length();k++){
					var temp=dragonBones.addTextureToSWFItem(armature.b[j].d[k].@name) 
					textureAtlasXML.appendChild(temp)
				}
			}
		}
	...
	dragonBones.initunGuide()

	return ret.toXMLString();
	 
}
   
dragonBones.getUIEditorAllAttr=function(){ 
	var doc=fl.getDocumentDOM();  
	var lib=fl.getDocumentDOM().library;  
	var tl=fl.getDocumentDOM().getTimeline();  
	...
	for(var i=0;i<doc.selection.length;i++){ 
		...
		var xml=new XML( doc.selection[i].actionScript);
		xml.@name=doc.selection[i].libraryItem.name;
 ...
 dragonBones.setUIEditorAttr=function(mc,item,key,data){ 
	var doc=fl.getDocumentDOM();  
	var lib=fl.getDocumentDOM().library;  
	var tl=fl.getDocumentDOM().getTimeline();  
	mc=getUIEditorMc(mc);
	...
	itemXml=getUIEditorAttrItem(xmlData,item,key); 
	if(itemXml==null){
		itemXml=<item name={item} key={key} data={data} />
		xmlData.appendChild(itemXml);
		 fl.trace("add:"+itemXml.toXMLString())
	}else{
		itemXml.@data=data;
		 fl.trace("change:"+itemXml.toXMLString())
	} 
	mc.actionScript=xmlData.toXMLString(); 
}


function getUIEditorAttrItem(xmlData,item,key){
	var doc=fl.getDocumentDOM();  
	var lib=fl.getDocumentDOM().library;  
	var tl=fl.getDocumentDOM().getTimeline();  

	for each(var nodeItem in xmlData["item"]){
		 if(nodeItem.@name==item && nodeItem.@key==key){
			 return nodeItem;
		 } 
	}
...
function getUIEditorMc(name){
	...
	for(var i=0;i<doc.selection.length;i++){ 
		...
		if(doc.selection[i].libraryItem.name==name){
			return  doc.selection[i];
		}
	}
	return null;
...
dragonBones.initunGuide=function(){  
fl.trace("initunGuide")
var doc=fl.getDocumentDOM();  
var lib=fl.getDocumentDOM().library;  
var tl=fl.getDocumentDOM().getTimeline();  
...
function getFileExt(fileName){
     ...
     var fname=fileName.substr(0,ind); 
     ext=ext.toLowerCase();  
	 return ext;
}
...
{  
        ...
        {  
                ...
                var pngFolder="png"+folderPath.split("images")[1];  
                if( lib.selectItem(pngFolder+"/"+fileName)){
					fl.trace("exit "+(pngFolder+"/"+fileName))
					continue;
...
　　			    lib.selectItem(fileName);
                lib.moveToFolder(pngFolder,fileName); 
				 
				var graphicFolder="images"+folderPath.split("images")[1];  
                var graphicItem=graphicFolder+"/"+fname;    
				lib.addNewItem("Graphic",graphicItem);  
				lib.editItem(graphicItem);  
　　			    lib.selectItem(pngFolder+"/"+fileName);//选中位图
        ...
        //子文件夹  
         ...
         {  
               analyseFolder(folderPath+"/"+folderList[j]);  
         }  
}  

dragonBones.importDirect=function(){  
fl.outputPanel.clear();   
var doc=fl.getDocumentDOM();  
var lib=fl.getDocumentDOM().library;  
var urlFolder = fl.browseForFolderURL("选择需要导入的文件夹");
if(urlFolder)
{ 
	analyseFolder(urlFolder);
}
else 
{
	alert("请选择有效的文件夹");
