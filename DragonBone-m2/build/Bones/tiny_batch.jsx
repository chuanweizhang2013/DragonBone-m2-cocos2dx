...
//
// $Id: xbatch.jsx,v 1.55 2014/05/01 18:33:49 anonymous Exp $
...
function main() {
  ...
  if (opts) {
    ...
    var actions=Stdlib.getActions("compress");

    for(var i=0;i<actions.length;i++){ 
        var resourceDir=opts.source;
         var outDir=opts.outf;
         if(actions[i]=="ani_images"){
                resourceDir=resourceDir+"/ani_images"; 
                outDir=outDir+"/ani_images"; 
         }else{  
                resourceDir=resourceDir+"/images/"+ actions[i]; 
                outDir=outDir+"/images/"+ actions[i]; 
        }
        ...
        batchOpts.overrideOpen=true;
        batchOpts.script = opts.script;
        xbatch(resourceDir,actions[i], opts.actionSet, batchOpts);
    } 
  }
};
